...
class Mood  : AppCompatActivity() {
    ...
    inner class MainLogic(question: String, question2: String) {
        ...
        val intent = Intent(this@Mood, Results_display::class.java)
    ...
    //플라스트 서버 연동
    private fun sendPostRequest(userInput: String) {
        val apiService = RetrofitClient.instance_4
        val request = PredictionRequest(text = userInput)
        ...
        apiService.getPrediction(request).enqueue(object : Callback<PredictionResponse> {
            override fun onResponse(call: Call<PredictionResponse>, response: Response<PredictionResponse>) {
                if (response.isSuccessful) {
                    ...
                    Log.d("MyTag", "코벌트 감정결과값: $prediction")



                    val appendedText_movie = "아무런 코멘트 없이 오로지 영화제목만 10개 말해줘."
                    val questionText_movie = "$prediction $appendedText_movie"

                    val appendedText_tv = "아무런 코멘트 없이 오로지 드라마제목만 10개 말해줘."
                    val questionText_tv = "$prediction $appendedText_tv"
